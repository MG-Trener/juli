(function(){
  ...
  async function openProgress(studentId){
    ...
    body.innerHTML=[1,2,3,4,5,6,7].map(level=>{const allowed=accessSet.has(level);return `<div class="progress-course ${allowed?'':'locked'}"><div class="progress-course-head"><strong>${level}. ${courses[level]}</strong><span>${allowed?'Доступ открыт':'Нет доступа к курсу'}</span></div><div class="progress-modules">${[1,2,3,4,5,6].map(n=>`<label class="progress-module"><input type="checkbox" data-level="${level}" data-module="${n}" ${done.has(`${level}-${n}`)?'checked':''} ${allowed?'':'disabled'}> Модуль ${n}</label>`).join('')}</div></div>`}).join('');
    status.textContent='Отметьте только те модули, которые преподаватель действительно принял.';
  }

  save.onclick=async()=>{
    if(!currentStudent)return;save.disabled=true;status.className='progress-status';status.textContent='Сохраняем...';
    const checks=[...body.querySelectorAll('input[data-level]')].filter(x=>!x.disabled);
    const now=new Date().toISOString();
    const rows=checks.map(x=>({student_id:currentStudent,course_level:Number(x.dataset.level),module_no:Number(x.dataset.module),completed:x.checked,completed_at:x.checked?now:null}));
    const {error}=await db.from('student_progress').upsert(rows,{onConflict:'student_id,course_level,module_no'});
    save.disabled=false;
    ...
    status.className='progress-status ok';status.textContent='✓ Прогресс ученика успешно сохранён';
    save.textContent='Сохранено ✓';setTimeout(()=>save.textContent='Сохранить прогресс',1800);
  };

  function inject(){
    document.querySelectorAll('.student[id^="student-"]').forEach(card=>{
      const id=card.id.slice(8);const target=card.querySelector('.actions-right');if(!target||target.querySelector('.progress-admin-btn'))return;
      const btn=document.createElement('button');btn.type='button';btn.className='progress-admin-btn';btn.textContent='Прогресс';btn.onclick=()=>openProgress(id);target.insertBefore(btn,target.firstChild);
    });
  }
