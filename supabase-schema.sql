...
-- SECURITY DEFINER helpers are for RLS/trigger usage, not anonymous RPC calls.
revoke execute on function public.handle_new_user() from public,anon,authenticated;
revoke execute on function public.is_superteacher() from anon;
revoke execute on function public.is_teacher() from anon;
revoke execute on function public.teacher_has_student(uuid) from anon;
revoke execute on function public.teacher_has_course(integer) from anon;
revoke execute on function public.teacher_can_manage_student_course(uuid,integer) from anon;
